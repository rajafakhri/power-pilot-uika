        ...
        if($sum_gen_usage >= 0){
            ...
            if($battery_user_imp == TRUE){
                ...
                if($kurang_bat <= 0){                    
                    ...
                    // Cari Battery Lain yang Dapat Menampung Listrik
                    $battery_user2 = DB::table('battery')->where('id_users',$id)->where('bat_watt','>',0)->first();
                    if($battery_user2 == TRUE){
                        $kurang_bat2 = $battery_user2->bat_watt + $kurang_bat;
                        if($kurang_bat2 <= 0){
                            $update_bat2 = DB::table('battery')->where('id_battery',$battery_user2->id_battery)->update([
                                'bat_watt' => 0,
                                'residu_val' => $battery_user_imp->capacity,
                            ...
                            // Jika Masih Mampu
                            $residu_batt_imp2 = $battery_user2->capacity - $kurang_bat2;
                            $update_bat2 = DB::table('battery')->where('id_battery',$battery_user2->id_battery)->update([
